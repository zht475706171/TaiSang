...
def test_edit_tool(tmp_path):
    ...
    tool = EditTool(source_root=tmp_path, confirmer=AutoApproveConfirmer())
...
def test_edit_path_traversal(tmp_path):
    """../../etc/passwd 应被 _is_within 拒,返回 path outside cwd。"""
    tool = EditTool(source_root=tmp_path, confirmer=AutoApproveConfirmer())
...
def test_edit_user_denies(tmp_path):
    ...
    tool = EditTool(source_root=tmp_path, confirmer=AutoDenyConfirmer())
    result = tool.run({"file_path": "a.py", "old_string": "pass", "new_string": "return 1"})
    assert result["ok"] is False
...
def test_edit_file_not_found(tmp_path):
    """文件不存在 → 报错 file not found(不是 outside cwd,是文件不存在)。"""
    tool = EditTool(source_root=tmp_path, confirmer=AutoApproveConfirmer())
...
def test_write_tool(tmp_path):
    ...
    tool = WriteTool(source_root=tmp_path, confirmer=AutoApproveConfirmer())
...
def test_write_user_denies(tmp_path):
    ...
    tool = WriteTool(source_root=tmp_path, confirmer=AutoDenyConfirmer())
    result = tool.run({"file_path": "new.py", "content": "print('hi')\n"})
    assert result["ok"] is False
    assert "user denied" in result["error"]
...
def test_write_path_traversal(tmp_path):
    """../../tmp/x 应被 _is_within 拒,返回 path outside cwd。"""
    tool = WriteTool(source_root=tmp_path, confirmer=AutoApproveConfirmer())
