...
Plan 1 完成后,`code-reader` 只支持 `code-reader index <repo_url>` 走 `git clone` 远程仓库。问题:

1. **用户场景错位**:真实用户想看一个项目,通常是先 `git clone` 到本地再看。强制 `code-reader` 自己 clone 反人类——用户磁盘里会莫名其妙多一堆 `~/.code-reader/cache/<hash>/` 目录。
2. **落盘位置不合理**:索引产物集中落 `~/.code-reader/indices/<hash>/`,hash 不可读,用户想看"我索引过哪些 repo""某 repo 的摘要是啥"只能人肉翻文件 / 翻 SQLite。
3. **`Fetcher` 过度设计**:`git clone` + retry + stale target 清理 + cache_dir hash 分子目录——这套逻辑用户根本不需要。

## 二、目标

**核心改动**:`code-reader` 改成只支持本地 repo,索引产物落 `<repo>/.code-reader/`。
...
改动前:
  CLI index <url> → IndexerService.build(url)
    → Fetcher.fetch(url) → git clone 到 ~/.code-reader/cache/<hash>/
    → parser → linker → Storage(~/.code-reader/indices/<hash>/ast.db)
    → Summarizer → ~/.code-reader/indices/<hash>/repo_map.json

改动后:
  CLI index <path> → IndexerService.build(path)
    → Fetcher.current_commit(path) → 直接用 path,不 clone
    → parser → linker → Storage(<path>/.code-reader/ast.db)
    → Summarizer → <path>/.code-reader/repo_map.json
```

### 3.2 `~/.code-reader/` 改动后只剩 `settings.json`(LLM 配置),`cache/` 和 `indices/` 目录不再创建。
...
| `PathManager` | 管理 `~/.code-reader/cache` + `indices/<hash>` | 管理 `<repo>/.code-reader/`,删 cache/indices 相关 |
| `IndexerService` | 调 `fetcher.fetch` 拿 local_path | 接收 `source_root` 直接用,commit 从 `Fetcher.current_commit` 取 |

## 四、组件改动细节

### 4.1 `Fetcher`(`src/code_reader/indexer/fetcher.py`)
...
### 4.2 `PathManager`(`src/code_reader/storage/paths.py`)
...
### 4.3 `IndexerService`(`src/code_reader/indexer/service.py`)
...
**扫源码新增排除 `.code-reader/`**(防止把索引产物当源码解析):
```python
py_files = [
    f for f in py_files
...
### 4.4 `RepoIndex`(`src/code_reader/types.py`)
...
### 4.5 CLI(`src/code_reader/cli/main.py`)
...
只在 `code-reader index` 末态打印一行提示:`建议把 .code-reader/ 加到 .gitignore`。不主动改用户的 `.gitignore` 文件。
...
```
用户: code-reader index D:/GoProject/wwBuy
  │
  ...
  │    │    → <source_root>/.code-reader/ast.db
  │    ├─ 扫所有 *.py(排除 .git + .code-reader)
  ...
  │    → <source_root>/.code-reader/repo_map.json
  └─ click.echo 提示:建议把 .code-reader/ 加到 .gitignore

用户: code-reader ask "main 调了谁" --repo D:/GoProject/wwBuy
  │
...
| `.code-reader/ast.db` 损坏 | 不存在此场景 | SQLite 打开失败抛 `DatabaseError`,CLI 层捕获提示删 `.code-reader/` 重建 |

### 5.3 向后兼容性

**不向后兼容**:本次改造是破坏性改动。
- `RepoIndex.repo_url` 字段改名 `source_root` — 旧 `repo_map.json` 反序列化会失败
- CLI 参数语义变了 — 旧脚本 `code-reader index <url>` 会当 url 是本地路径,`is_dir()` 失败

**处理**:不写迁移脚本。旧 `~/.code-reader/cache/` + `indices/` 目录留垃圾,不主动清(用户可手动删)。
...
- `test_index_dir_creates_code_reader_subdir` — `index_dir(tmp_path)` 返回 `tmp_path/.code-reader` 且自动创建
- `test_index_db_path_under_code_reader` — 路径是 `tmp_path/.code-reader/ast.db`
- `test_repo_map_path_under_code_reader` — 路径是 `tmp_path/.code-reader/repo_map.json`
...
原来用 `https://github.com/...` URL,改成在 `tmp_path` 下建几个 .py 文件,然后 `code-reader index <tmp_path>` / `ask --repo <tmp_path>`。

### 6.7 新增:扫源码排除 `.code-reader/` 测试

- `test_indexer_excludes_code_reader_dir` — 在 `tmp_path/.code-reader/fake.py` 放个 Python 文件,断言它不被解析进 `idx.files`
...
- 改:约 10 个(改参数,逻辑不变)
- 新增:`test_fetcher.py` 3 + `test_paths.py` 4 + `test_cli.py` 2 + `test_indexer_excludes_code_reader_dir` 1 = 10 个
...
单个 commit,消息:`refactor: drop remote clone, support local repo only + land index under <repo>/.code-reader/`
...
3. **用户误把 `code-reader index <url>` 当旧用法**:URL 不存在本地,`is_dir()` 失败,exit 1 + 提示"路径不存在"。错误信息可加一句"v1 不再支持远程 clone,请先 `git clone` 到本地再 index"。
4. **`<repo>/.code-reader/` 已被用户占用**:理论上不会,但若用户正好有同名目录,`PathManager.index_dir` 会 `mkdir(exist_ok=True)` 不报错,`IndexStorage` 会建 `ast.db`。可接受。
